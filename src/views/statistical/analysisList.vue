        ...
        <el-tabs v-model="activeName" type="card" style="backgroundColor:inherit;">
            ...
            <el-tab-pane label="单台情况" name="single" v-loading="loading">
                <div class="header_l">
                    ...
                    <el-select v-model="ChooseLift" filterable placeholder="请选择" style="margin: 0 10px" clearable
                               @change="handleSelect">
                        <el-option
                ...
                <paginate :api="paginate_api" :params="params" @val-change="onsingleValChange"
                          :refresh="refresh"></paginate>
            </el-tab-pane>
        </el-tabs>
    </div>
    ...
    export default {
        ...
        methods: {
            ...
            initChart(date, dom, id) {
                this.$api_v3.post('LogOnline/offlineStatistic', {
                    start_date: date[0],
                    end_date: date[1],
                    lift_id: id
                }).then((res) => {
                    if (res.code === 0) {
                        ...
                        res.data.forEach((i) => {
                            xArr.push(i.date)
                            yArr.push(i.number)
                            totalOffline += i.number
                        })
                        this.totaloff = totalOffline;
                        if (id) {
                            ...
                            this.$echarts.init(document.getElementById(dom[0])).setOption({
                                xAxis: {data: xArr},
                                series: [{data: yArr}]
                            })
                        }
                ...
                this.$api_v3.post('LogOnline/offlineTimeStatistic', {
                    start_date: date[0],
                    end_date: date[1],
                    lift_id: id
                }).then((res) => {
                    if (res.code === 0) {
                        ...
                        if (id) {
                            ...
                            this.$echarts.init(document.getElementById(dom[1])).setOption({
                                xAxis: {data: xArr},
                                series: [{data: yArr}]
                            })
